...
for make in makes:
  for model in models:
    if make[0] == model[2]:
      car_colors = [colorz for colorz in available_car_colors if colorz[0] == model[0]]
      color_names = []
      for color_name in car_colors:
        for color_tuple in colors:
          if color_tuple[0] == color_name[1]:
            color_names.append(color_tuple[1])
      car_dict[make[1]][model[1]] = color_names
